...
let build_run_spec
  ...
  let base =
    let hash = Stdlib.Option.(hash |> map (fun h -> "@" ^ h) |> value ~default:"") in
    let image = Printf.sprintf "ocaml/opam:%s%s" (Variant.docker_tag variant) hash in
    Spec.Docker image
  in
  let config = Spec.opam ~variant ~lower_bounds ~with_tests ~opam_version:`Dev pkg in
...
let variant =
  ...
  and+ compiler =
    let default =
      Ocaml_version.(to_string (with_just_major_and_minor Releases.latest)), None
    in
    let info =
      Arg.info [ "compiler" ]
        ~doc:"TODO"
...
let build_cmd =
  ...
  let term =
    ...
    and+ pkg = pkg_term
    and+ opam_repository = local_opam_repo_term
    in
    build_run_spec ~variant ~hash ~no_cache ~with_tests ~lower_bounds ~pkg ~opam_repository
  in
  let info =
